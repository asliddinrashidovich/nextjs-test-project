...
function ProductsClientSide({products}: {products: ProductTypes[]}) {
  ...
  return (
    ...
    {products.map(item => (
        <div key={item.id} onClick={() => router.push(`products/${item.id}`)}  className='border-[1px] relative rounded-[5px] p-[20px] cursor-pointer'>
            ...
            <div className='flex justify-between items-center left-0 right-0 px-[20px] absolute bottom-[20px]'>
                ...
                <button className='p-[5px] active:bg-[#5ed1ff] cursor-pointer text-[#fff] rounded-[5px] bg-[#00b7ff]'>Add to cart</button>
            </div>
        </div>
    ))}
</div>
